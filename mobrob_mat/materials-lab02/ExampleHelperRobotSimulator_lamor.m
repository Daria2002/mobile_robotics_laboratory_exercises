classdef ExampleHelperRobotSimulator_lamor < handle
    %ExampleHelperRobotSimulator_lamor Simulator for simple differential-drive robot
    ...
    %
    %   OBJ = ExampleHelperRobotSimulator_lamor creates a simple simulator for a
    ...
    %
    %   OBJ = ExampleHelperRobotSimulator_lamor(MAPNAME) loads a predefined map
    ...
    %
    %   OBJ = ExampleHelperRobotSimulator_lamor(MAPNAME,MAPRESOLUTION) loads a
    %   predefined map with MAPNAME and sets the resolution to
    %   MAPRESOLUTION (in cells / meter). By default, the resolution is 2
    %   cells / meter.
    %
    %   OBJ = ExampleHelperRobotSimulator_lamor(BOG) loads the map defined by the
    %   robotics.BinaryOccupancyGrid object BOG into the simulator.
    %
    %   Notes:
    %   1) ExampleHelperRobotSimulator_lamor's ROS interface expects MATLAB ROS
    ...
    %
    %   ExampleHelperRobotSimulator_lamor Published Topics:
    ...
    %
    %   ExampleHelperRobotSimulator_lamor Subscribed Topics:
    %       /mobile_base/commands/velocity - Velocity commands to the robot
    %
    %   ExampleHelperRobotSimulator_lamor Services:
    ...
    %   various properties.
    %
    %   ExampleHelperRobotSimulator_lamor properties:
    %
    ...
    %
    %   ExampleHelperRobotSimulator_lamor methods:
    %
    ...
    %
    %     % Create Simulator
    %     robotsim = ExampleHelperRobotSimulator_lamor;
    %
    ...
    %     rosshutdown
    %
    %   See also robotics.BinaryOccupancyGrid, ExampleHelperSimRangeSensor_lamor, ExampleHelperSimDifferentialDriveRobot.
    ...
    properties(Constant, Access = protected)
        ...
        FigureTag = 'ExampleHelperRobotSimulator_lamor'
    end
    ...
    properties (Access = {?ExampleHelperRobotSimulator_lamor, ?matlab.unittest.TestCase})
    ...
    methods
        function obj = ExampleHelperRobotSimulator_lamor(varargin)
            %ExampleHelperRobotSimulator_lamor Constructor for simulator object
            %   Please check the class definition for details.
            
            obj.MyFigureTag = ExampleHelperRobotSimulator_lamor.FigureTag;
            ...
            examplesFilePath = fullfile(fileparts(mfilename('fullpath')), 'data', 'robotSimulatorExampleMaps.mat');
            ...
            switch nargin
                case 0
                    % ExampleHelperRobotSimulator_lamor() syntax
                    % Use default values
                    mapName = 'emptyMap';
                    mapResolution = 2;
                ...
                case 1
                    % ExampleHelperRobotSimulator_lamor(MAPNAME) or
                    % ExampleHelperRobotSimulator_lamor(BOG) syntax
                    map = varargin{1};
                    if ischar(map)
                        % ExampleHelperRobotSimulator_lamor(MAPNAME) syntax
                        mapResolution = 2;
                        mapName = validatestring(map, fieldnames(exampleMaps), 'ExampleHelperRobotSimulator_lamor', 'mapName');
                        obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                    else
                        % ExampleHelperRobotSimulator_lamor(BOG) syntax
                        validateattributes(map, {'robotics.BinaryOccupancyGrid'}, {'nonempty','scalar'}, ...
                            'ExampleHelperRobotSimulator_lamor', 'bog');
                        obj.InternalMap = map;
                    end
                case 2
                    % ExampleHelperRobotSimulator_lamor(MAPNAME, MAPRESOLUTION) syntax
                    mapName = varargin{1};
                    mapResolution = varargin{2};
                    mapName = validatestring(mapName, fieldnames(exampleMaps), 'ExampleHelperRobotSimulator_lamor', 'mapName');
                    validateattributes(mapResolution, {'double'}, {'nonempty','scalar'}, 'ExampleHelperRobotSimulator_lamor', 'mapResolution');
                    obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                otherwise
                    error('RobotSimulator:InvalidInput',...
                        'Invalid number of arguments to ExampleHelperRobotSimulator_lamor.');
            end
    ...
    methods (Access = private)
        ...
        function setupLaserScanner(obj)
            ...
            obj.LaserSensor = ExampleHelperSimRangeSensor_lamor();
            obj.LaserSensor.Map = obj.InternalMap;
